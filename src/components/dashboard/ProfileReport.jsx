...
export const ProfileReport = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (yearlyPerformance && yearlyPerformance.length > 0) {
          ...
          const sortedYearlyData = [...yearlyPerformance].sort(
            (a, b) => b.year - a.year
          );
  ...
  const totalRevenue =
    currentYearData?.monthlyRevenue.reduce(
  ...
  const calculateGrowth = () => {
    ...
    if (yearlyData.length >= 2) {
      ...
      const previousYearTotal =
        previousYearData?.monthlyRevenue.reduce(
          (sum, month) => sum + month.revenue,
          0
        ) || 0;
            ...
            <div className="d-flex align-items-center justify-content-between">
              <div className="d-flex flex-column">
                ...
                <div className="d-flex align-items-baseline">
                  <h3 className="display-6 fw-bold mb-0 me-2">
                    ₹{(totalRevenue / 1000).toFixed(1)}k
                  </h3>
                  <div
                    className={`badge ${
                      growthPercentage >= 0 ? "bg-success" : "bg-danger"
